...
(function () {
  ...
  function init() {
    ...
    renderer = new THREE.WebGLRenderer({ antialias: true,alpha: true,canvas: document.getElementById("bg") });
  ...
  function animate() {
    ...
    for (let i = 0; i < confettiNum; i++) {
      ...
      if (dummy.position.y < -worldRadius) {
        ...
        if (i % 4 === 1) dummy.position.x += speed_x,dummy.position.z += speed_z;
        else if (i % 4 === 2) dummy.position.x += speed_x,dummy.position.z -= speed_z;
        else if (i % 4 === 3) dummy.position.x -= speed_x,dummy.position.z += speed_z;
        else dummy.position.x -= speed_x,dummy.position.z -= speed_z;
      }
